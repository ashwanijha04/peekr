...
class PIIRedact(BaseGuardrail):
    ...
    def __init__(
        ...
        if categories is not None:
            ...
            if unknown:
                raise ValueError(
                    f"Unknown PII categories: {unknown}. Valid: {set(_PII_PATTERNS)}"
                )
    ...
    * ``"warn"``   — records to ``guardrail_warnings`` without modifying
    ...
    def __init__(
        ...
        if action not in ("raise", "redact", "warn"):
            raise ValueError(
                f"action must be 'raise', 'redact', or 'warn'; got {action!r}"
            )
    ...
    def run(self, span: "Span") -> None:
        ...
        if score < self.threshold:
            ...
            if self.action == "raise":
                raise GuardrailError(
                    msg, guardrail_name="HallucinationBlock", span=span
                )
...
class _BlockingGuardrailExporter:
    ...
    def __init__(self, guardrails: list[BaseGuardrail]) -> None:
        self.guardrails = [
            g for g in guardrails if g._blocks and not getattr(g, "_input_guard", False)
        ]
